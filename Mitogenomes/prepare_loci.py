...
def command_split(args: argparse.Namespace) -> None:
    ...
    audit_path = output_dir / "extraction_audit.tsv"
    with audit_path.open("w") as audit:
        audit.write("sample\tlocus\tlength\n")
        for locus in LOCI:
            ...
            with (output_dir / f"{locus}.fasta").open("w") as out:
                for sample in samples:
                    sequence = grouped[locus][sample]
                    write_wrapped_fasta(out, sample, sequence)
                    audit.write(f"{sample}\t{locus}\t{len(sequence)}\n")
