#!/usr/bin/env python3
"""Validate, concatenate, partition, and root mitochondrial alignments."""
...
def command_validate(args: argparse.Namespace) -> None:
    ...
    with output.open("w") as out:
        out.write("locus\tn_taxa\talignment_length\tn_gap_characters\tn_missing_characters\n")
        for locus in LOCI:
            records, length = validate_one(input_dir / f"{locus}.fasta", samples)
            gaps = sum(sequence.count("-") for sequence in records.values())
            missing = sum(sequence.count("?") + sequence.count("N") for sequence in records.values())
            out.write(f"{locus}\t{len(records)}\t{length}\t{gaps}\t{missing}\n")
    print(f"Validated {len(LOCI)} alignments in {input_dir}")
...
def command_concat(args: argparse.Namespace) -> None:
    ...
    output_nexus = Path(args.output_nexus).resolve()
    output_partitions = Path(args.output_partitions).resolve()
    output_audit = Path(args.output_audit).resolve()
    for path in (output_fasta, output_nexus, output_partitions, output_audit):
    ...
    with output_nexus.open("w") as out:
        out.write("#NEXUS\nBEGIN DATA;\n")
        out.write(f"  DIMENSIONS NTAX={len(samples)} NCHAR={total_length};\n")
        out.write("  FORMAT DATATYPE=DNA MISSING=? GAP=-;\n  MATRIX\n")
        for sample, sequence in combined_records:
            out.write(f"  {sample}  {sequence}\n")
        out.write("  ;\nEND;\n")

    start = 1
    with output_partitions.open("w") as partitions, output_audit.open("w") as audit:
        audit.write("locus\tstart\tend\talignment_length\n")
        for locus, _, length in alignments:
            end = start + length - 1
            partitions.write(f"DNA, {locus} = {start}-{end}\n")
            audit.write(f"{locus}\t{start}\t{end}\t{length}\n")
            start = end + 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate.add_argument("--samples-file", required=True)
    ...
    concat.add_argument("--output-nexus", required=True)
    concat.add_argument("--output-partitions", required=True)
    concat.add_argument("--output-audit", required=True)
