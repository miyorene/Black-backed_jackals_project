...
#SBATCH --job-name=mitochondrial_phylogeny
...
MITO_DIR="${BASE}/mitochondrial"
ORIGINAL_DIR="${MITO_DIR}/original"
RESULTS_DIR="${MITO_DIR}/complete_pipeline"
...
python3 "$CONCATENATE_SCRIPT" \
    --output-fasta "${CONCAT_DIR}/mitochondrial_mafft_concatenated.fasta" \
    --output-partitions "${CONCAT_DIR}/mitochondrial_mafft.partitions" \
    --sample-prefix-order LMS,LMM,LAD \
    "${MAFFT_ALIGNMENTS[@]}"

python3 "$CONCATENATE_SCRIPT" \
    --output-fasta "${CONCAT_DIR}/mitochondrial_gblocks_concatenated.fasta" \
    --output-partitions "${CONCAT_DIR}/mitochondrial_gblocks.partitions" \
    --sample-prefix-order LMS,LMM,LAD \
    "${GBLOCKS_ALIGNMENTS[@]}"
...
iqtree3 \
    ...
    -p "${CONCAT_DIR}/mitochondrial_mafft.partitions" \
    -m MFP \
    -B 1000 \
    -o LAD_1 \
    -T AUTO \
    -ntmax "$THREADS" \
    -seed 20260802 \
    --prefix "${IQTREE_DIR}/mitochondrial_mafft"

iqtree3 \
    -s "${CONCAT_DIR}/mitochondrial_gblocks_concatenated.fasta" \
    -p "${CONCAT_DIR}/mitochondrial_gblocks.partitions" \
    -m MFP \
    -B 1000 \
    -o LAD_1 \
    -T AUTO \
    -ntmax "$THREADS" \
    -seed 20260802 \
    --prefix "${IQTREE_DIR}/mitochondrial_gblocks"
